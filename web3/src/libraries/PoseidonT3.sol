...
library PoseidonT3 {
    ...
    function getZeroHash(uint256 level) internal pure returns (uint256) {
        ...
        // For levels > 4, compute a deterministic zero hash
        // Using a simple but deterministic formula for MVP
        return uint256(keccak256(abi.encodePacked("zero_hash_level", level))) >> 8;
    }
}