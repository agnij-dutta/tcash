// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;

interface IShieldedVault {
    event CommitmentInserted(bytes32 indexed commitment, uint32 index, bytes32 newRoot);
    event NullifierUsed(bytes32 indexed nullifier);
    event RootUpdated(bytes32 indexed newRoot);
    event SpendAccepted(bytes32 indexed nullifier, address indexed tokenIn, uint256 amountIn);

    error InvalidDenomination();
    error InvalidRoot();
    error NullifierAlreadyUsed();
    error Unauthorized();

    // Legacy deposit removed in favor of EERC Converter flows

    // Legacy withdraw removed in favor of EERC Converter flows

    // Execute private spend record and escrow movement for Router
    function executeSpend(
        bytes calldata proof,
        bytes32 root,
        bytes32 nullifier,
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
        uint256 minAmountOut
    ) external returns (uint256 amountOut);

    function latestRoot() external view returns (bytes32);
}


