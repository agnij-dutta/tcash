...
type RegistrationStatus = 'not_started' | 'key_generated' | 'registered' | 'failed'
...
export function useEERC() {
  ...
  const [registrationStatus, setRegistrationStatus] = useState<RegistrationStatus>('not_started')
  const [transactionHash, setTransactionHash] = useState<string | undefined>()
  const [logs, setLogs] = useState<Array<{ timestamp: Date, level: 'info' | 'warn' | 'error', message: string, data?: any }>>([])
  ...
  // Debug circuit URLs
  useEffect(() => {
    addLog('info', 'Circuit URLs configuration', { circuits: CIRCUIT_URLS })
  }, [addLog])
  ...
  }, [address, publicClient, walletClient, decryptionKey, eERC.isInitialized, registrationStatus])
  ...
  useEffect(() => {
    ...
    const storedStatus = SecureStorage.retrieve(STORAGE_KEYS.REGISTRATION_STATUS) as RegistrationStatus
    if (storedStatus) {
      setRegistrationStatus(storedStatus)
      addLog('info', 'Registration status loaded from storage', { status: storedStatus })

      // If stored status says registered, verify with blockchain
      if (storedStatus === 'registered' && publicClient && !hasCheckedRegistration.current) {
        hasCheckedRegistration.current = true
        checkRegistrationStatus().then(isRegisteredOnChain => {
          if (!isRegisteredOnChain) {
            // Stored status is incorrect, reset it
            addLog('warn', 'Stored registration status incorrect, user not registered on-chain')
            setRegistrationStatus('not_started')
            SecureStorage.store(STORAGE_KEYS.REGISTRATION_STATUS, 'not_started')
          } else {
            addLog('info', 'Registration status verified on-chain')
          }
        }).catch(error => {
          addLog('error', 'Failed to verify registration status', { error })
        })
      }
    } else {
      ...
      if (publicClient && !hasCheckedRegistration.current) {
        ...
        setTimeout(() => {
          checkRegistrationStatus().then(isRegisteredOnChain => {
            if (isRegisteredOnChain) {
              addLog('info', 'Found existing registration on-chain')
  ...
  const handleRegister = useCallback(async () => {
    ...
    try {
      ...
      addLog('error', 'Registration failed', { error: errorMessage, address })
  ...
  const handleGenerateDecryptionKey = useCallback(async () => {
    ...
    try {
      ...
      SecureStorage.store(STORAGE_KEYS.REGISTRATION_STATUS, 'key_generated')

      return key
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      addLog('error', 'Key generation failed', { error: errorMessage, address })
      setRegistrationStatus('failed')
      throw error
    }
  }, [address, eERC, addLog])
  ...
  const checkRegistrationStatus = useCallback(async (): Promise<boolean> => {
    if (!address || !publicClient || isCheckingRegistration) {
      if (!address || !publicClient) {
        addLog('warn', 'Cannot check registration: missing address or client')
      }
      return false
    }
    ...
    try {
      addLog('info', 'Checking on-chain registration status', { address })
      ...
      addLog('info', 'Registration status checked', { address, isRegistered })
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error'
      addLog('error', 'Failed to check registration status', { error: errorMessage, address })
      return false
    } finally {
  ...
  return {
    ...
    // Computed properties
    canRegister: !!address && !!walletClient && eERC.isInitialized && registrationStatus === 'key_generated',
    canGenerateKey: !!address && !!walletClient && eERC.isInitialized && registrationStatus === 'not_started',
