...
export default function TsunamiDashboard() {
  ...
  // Convert decrypted balance to human-readable format
  const balanceInTokens = useMemo(() => {
    if (!decryptedBalance || !erc20Decimals) return 0
    return Number(decryptedBalance) / Math.pow(10, erc20Decimals)
  }, [decryptedBalance, erc20Decimals])
  ...
  const tokens: TokenRow[] = useMemo(() => {
    const allTokens = [
      { 
        ...
        balance: balanceInTokensFormatted, 
        usd: balanceInTokensFormatted, 
        icon: DollarSign 
      },
      // Future encrypted tokens would go here when implemented
  ...
  // Analytics: compute allocation for pie chart (only non-zero balances)
  const allocations = tokens.filter(t => t.usd > 0).map((t) => ({ symbol: t.symbol, value: t.usd }))
        ...
        <div className="text-center max-w-md">
          ...
          <button
            onClick={async () => {
              try {
                await register()
                // The component will re-render when isRegistered becomes true
              } catch (error) {
          ...
          <div className="mt-4 divide-y divide-white/10">
            {tokens.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <Wallet className="w-12 h-12 text-white/30 mb-3" />
                <div className="text-white/70 text-sm font-medium mb-1">No private tokens yet</div>
                <div className="text-white/50 text-xs">Deposit some tokens to get started</div>
              </div>
            )}
          </div>
        </section>
            ...
            <div className="space-y-4">
              {recentActivity.length > 0 ? (
                  ...
                  <div key={i} className="flex items-center justify-between py-3 border-b border-white/10 last:border-b-0">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-white/10 border border-white/15 flex items-center justify-center">
                        ...
                        {a.type === "Transfer" && <ArrowLeftRight className="w-4 h-4 text-blue-300" />}
                      </div>
                      <div>
                    ...
                    <div className="text-right">
                      ...
                      <div className={`text-xs font-medium ${a.status === "confirmed" ? "text-emerald-300" : a.status === "failed" ? "text-red-300" : "text-yellow-200"}`}>
                        {a.status}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <div className="text-white/70 text-sm font-medium mb-1">No transactions yet</div>
                  <div className="text-white/50 text-xs">Make your first deposit to get started</div>
                </div>
              )}
            </div>
            ...
            <section className="backdrop-blur-xl border border-white/15 rounded-2xl p-6 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.06),0_16px_56px_rgba(0,0,0,0.45)]" style={{ background: "rgba(255,255,255,0.02)" }}>
              ...
              {totalAlloc > 0 ? (
                <div className="grid grid-cols-2 gap-4 items-center">
                  ...
                  {(() => {
                    const swapTransactions = transactions.filter(tx => tx.type === 'swap' && tx.status === 'confirmed')
                    const swapCount = swapTransactions.length
                    const thisMonth = new Date().toLocaleDateString('en-US', { month: 'long' })
                    return (
                      <div className="mt-4 text-xs text-white/60">
                        {swapCount > 0 ? `${swapCount} private swap${swapCount !== 1 ? 's' : ''} this month` : `No swaps yet this ${thisMonth}`}
                      </div>
                    )
                  })()}
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <PieChart className="w-16 h-16 text-white/30 mb-4" />
                  <div className="text-white/70 text-sm font-medium mb-1">No balances to analyze</div>
                  <div className="text-white/50 text-xs">Make a deposit to see your portfolio breakdown</div>
                </div>
              )}
            </section>
          </div>
